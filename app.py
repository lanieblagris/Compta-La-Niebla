...
if not st.session_state['connected']:
    ...
    if choice == "Tableau de bord":
        ...
        # ... (ajoute les autres tabs ici si besoin)

        st.markdown("---")
        st.write("### 📊 STATS SEMAINE (Détail)")
        try:
            df_stats = conn.read(worksheet="Rapports", ttl=0)
            if not df_stats.empty:
                df_stats['Date'] = pd.to_datetime(df_stats['Date'], errors='coerce')
                # Tableau récapitulatif
                week_data = df_stats[~df_stats['Action'].str.contains(r'\[LOG\]', na=False)]
        ...
        # Ta logique de compta habituelle...
