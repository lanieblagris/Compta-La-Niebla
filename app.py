...
if not st.session_state['connected']:
    ...
    try:
        ...
        st.error("Erreur : Vérifiez les noms de vos onglets Google Sheets !")
        st.stop()
    ...
    # --- GESTION DES STOCKS ---
        ...
        with c2:
            with st.form("adj"):
                p = st.selectbox("Produit", DRUG_LIST); m = st.radio("Action", ["Ajout", "Retrait"]); q = st.number_input("Quantité", min_value=0.0)
                if st.form_submit_button("APPLIQUER"):
                    val = q if m == "Ajout" else -q
                    df_stock.loc[df_stock['Produit'] == p, 'Quantite'] += val
