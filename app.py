    ...
    if choice == "Tableau de bord":
        ...
        try:
            ...
            if not df_stats.empty:
                ...
                # 2. Tableau détaillé des actions
                st.write("#### 📈 Détail des activités par membre")
                if not week_data.empty:
                    pivot_df = week_data.groupby(['Membre', 'Action']).size().unstack(fill_value=0)
            ...
            with st.form(key=f"bl_{st.session_state.form_key}"):
                ...
                if st.form_submit_button("BLANCHIR"):
                    try:
                        ...
                        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                        op_s = {"Date": now, "Type": "Dépense", "Etat": "Sale", "Catégorie": "Blanchiment", "Montant": float(m_sale), "Note": "Sortie sale"}
                        op_p = {"Date": now, "Type": "Recette", "Etat": "Propre", "Catégorie": "Blanchiment", "Montant": float(propre), "Note": f"Retour propre (-{taux}%)"}
