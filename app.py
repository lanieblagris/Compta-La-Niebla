...
st.markdown("""
    ...
    h1, h2, h3, h4 { color: #ffffff; text-align: center; font-family: 'Courier New'; position: relative; z-index: 10; }
    .stForm { border: 1px solid #333; border-radius: 15px; background-color: rgba(20, 20, 20, 0.9); position: relative; z-index: 10; }
...
if not st.session_state['connected']:
    # PAGE DE CONNEXION AVEC BRUME
    st.markdown('<div class="fogwrapper"><div id="foglayer_01" class="fog"><div class="image01"></div><div class="image02"></div></div><div id="foglayer_02" class="fog"><div class="image01"></div><div class="image02"></div></div></div>', unsafe_allow_html=True)
    ...
    if choice == "Tableau de bord":
        ...
        st.markdown(f'<div style="width: 100%; overflow: hidden; margin-bottom: 10px;"><img src="{LOGO_URL}" style="width: 100%; height: 250px; object-fit: cover; border-radius: 10px; border: 2px solid #333;"></div>', unsafe_allow_html=True)
        ...
        def handle_submit(action, butin=0, drogue="N/A", quantite=0):
            try:
                new_row = pd.DataFrame([{
                    ...
                    "Action": action, "Drogue": drogue, "Quantite": float(quantite), "Butin": float(butin)
                }])
